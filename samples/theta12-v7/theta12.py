...
def main(args):
    include_future = not (args.exclude and "estimation" in args.exclude)
    #
    # RC params
    #
    ...
    if include_future:
        ax.tick_params(
            axis="y", direction="out", labelleft=True, labelright=False, pad=120
        )
    else:
        ax.tick_params(
            axis="y", direction="out", labelleft=True, labelright=False, pad=80
        )
