...
def main(args):
    ...
    colors = {
        "nova": "xkcd:bubblegum",
        "t2k": "xkcd:green teal",
        "icecube": "xkcd:clear blue",
        "juno": "xkcd:fire engine red",
        "pingu": "xkcd:vivid blue",
        "orca": "xkcd:charcoal grey",
        "hyperk": "xkcd:green",
        "dune": "xkcd:violet",
        "ino": "xkcd:orange",
        "hyperkkorea": "xkcd:emerald green",
        "ess": "xkcd:lilac",
    }
    ...
    for exp in args.inputs:
        ...
        # print(file)
        points = file["result"]["mh"]["year"]
        data_low = file["result"]["mh"]["low"]
        data_high = file["result"]["mh"]["high"]

        if isinstance(points[0], list):
            points = points[0]
        if isinstance(data_low[0], list):
            data_low = data_low[0]
        if isinstance(data_high[0], list):
        ...
        low = []
        high = []
        for (year_v, min, max) in zip(points, data_low, data_high):
            year_ax = year_v + start
            low.append((year_ax, min))
            high.append((year_ax, max))
        this_exp = {
            "id": exp.rsplit("/", 1)[-1].split("_", 1)[0].replace(".yaml", ""),
            "name": name,
            "start": start,
            "status": status,
            "low_values": low,
    ...
    for count, exp in enumerate(exps):
        ...
        poly = Polygon(
            exp["low_values"] + revert_max,
            facecolor=color,
            edgecolor=cs.colorConverter.to_rgba(color, alpha=0.1),
            alpha=alpha,
            lw=width,
            ls=lstyle,
        )
        ...
        plt.plot(
            [marker_place_x, sens_year_start],
            [marker_place_y, marker_place_y],
            ls="dotted",
            color=color,
            alpha=alpha,
        )
        ...
        if "juno" in expid:
            ...
            arrowoffset = fulloffset + oddoffset * 0.5
            # plt.text(start, -fulloffset-oddoffset, name, **textopts1)
            plt.arrow(2033.9, 2.4, -0.4, 0.8, **arrowprops)
        if "icecube" in expid:
            textopts1 = dict(textopts)
            textopts1["va"] = "bottom"
            arrowoffset = fulloffset + oddoffset * 0.5
