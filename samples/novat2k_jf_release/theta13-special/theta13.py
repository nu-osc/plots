...
def main(args):
    ...
    if args.exclude:
        ...
        result = result[mask]
    result = np.sort(result, axis=- 1, kind=None, order=("type", "measurement", "span"))
    result = result[::-1]
    ...
    for i, exp in enumerate(result):
        ...
        if measurement == 'estimation':
            name = f'\\makebox[{namewidth}]{{{{{font}{name}}} {{\\relsize{{-1}}({dataset}) {notes}}}{extra}'
        else:
            name = f'\\makebox[{namewidth}]{{{{{font}{name}}} \\hfill{{}}{notes}}}{extra}'
