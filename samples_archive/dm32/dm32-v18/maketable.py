...
def main(args):
    ...
    data = sorted(
        data, key=lambda item: item["span"] + 1.0e-8 * (100 - len(item["name"]))
    )
...
def collect_result(var, experiment):
    ...
    for res in results:
        ...
        if mode:
            val_left, val, val_right = convert(
                var, mode, val_left, val, val_right, context=context
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-v", "--variable", choices=variables, required=True, help="variable to read"
    )
